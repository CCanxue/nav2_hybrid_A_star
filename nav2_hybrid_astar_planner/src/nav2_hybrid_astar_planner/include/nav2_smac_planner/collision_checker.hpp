...
public:
    ...
    const bool & traverse_unknown)
  {
    ...
    if (!footprint_is_radius_) {
      ...
      if (traverse_unknown && footprint_cost_ == nav2_costmap_2d::NO_INFORMATION) {
        return false;
      }

      // if occupied or unknown and not to traverse unknown space
      return footprint_cost_ >= nav2_costmap_2d::LETHAL_OBSTACLE;
    } else {
      ...
      // if occupied or unknown and not to traverse unknown space
      return footprint_cost_ >= nav2_costmap_2d::INSCRIBED_INFLATED_OBSTACLE;
    }
  }
